function DashboardPage({ data }) {
  ...
  if (incompleteHabits.length > 0) {
    nextUp = { type: 'habit', name: incompleteHabits[0].name, time: incompleteHabits[0].time || 'Anytime', icon: '\u{1F525}' };
  } else if (upcomingTasks.length > 0) {
    var tk = upcomingTasks[0];
    nextUp = { type: 'task', name: tk.name, time: fmt(tk.dueDate), icon: '\u{1F4CB}' };
  }
  ...
  var greetingEmoji = hr < 12 ? '\u{1F305}' : hr < 17 ? '\u2600\uFE0F' : '\u{1F319}';
  ...
  return html`<div className="space-y-5 animate-in pb-6">
            ...
            <p className="text-sm text-gray-400 mt-1">
              ${habits.length === 0 && tasks.length === 0
                ...
                : completedToday === habits.length && habits.length > 0
                  ? 'All habits done! Crushing it today! \u{1F4AA}'
      ...
      ${nextUp && html`<div className="glass rounded-2xl p-4 border-l-4 border-accent flex items-center gap-4">
          ...
          <div className="w-full px-1">
            ...
            <div className="w-full h-2.5 bg-dark-700 rounded-full overflow-hidden">
              <div
                ...
                style=${{ width: waterPct + '%', background: waterPct >= 100 ? 'linear-gradient(90deg, #34d399, #10b981)' : 'linear-gradient(90deg, #60a5fa, #3b82f6)' }}
              />
            </div>
          </div>
      ...
      <div className="glass rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-bold text-white flex items-center gap-2">
            \u{1F4CA} Weekly Habit Completion
          </h2>
        ...
        <div className="flex items-end justify-between gap-2" style=${{ height: 140 }}>
          ${weeklyPcts.map(function(pct, i) {
            ...
            return html`<div key=${weekDates[i]} className="flex-1 flex flex-col items-center gap-1.5">
                ...
                <div className="w-full relative rounded-t-lg overflow-hidden" style=${{ height: Math.max(4, (pct / 100) * 90) + 'px' }}>
                  ...
                  />
                  ${isToday && html`<div className="absolute -top-0.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-accent rounded-full" />`}
                </div>
      ...
      ${activeGoals.length > 0 && html`<div className="glass rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-bold text-white flex items-center gap-2">
          ...
          <div className="space-y-3">
            ${activeGoals.map(function(goal) {
              ...
              return html`<div key=${goal.id} className="bg-dark-700/50 rounded-xl p-3 hover:bg-dark-700 transition">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-base">${goal.icon || '\u{1F3AF}'}</span>
                      <span className="text-sm font-semibold text-white truncate">${goal.name}</span>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      ${daysLeft !== null && html`<span className="${'text-[10px] px-2 py-0.5 rounded-full font-medium ' + (daysLeft < 0 ? 'bg-rose-500/15 text-rose-400' : urgent ? 'bg-amber-500/15 text-amber-400' : 'bg-dark-600 text-gray-400')}">
                  ...
                  <div className="w-full h-2 bg-dark-800 rounded-full overflow-hidden">
                    <div
                      ...
                      style=${{
                        width: pct + '%',
                        background: pct >= 100 ? 'linear-gradient(90deg, #34d399, #10b981)' : pct >= 60 ? 'linear-gradient(90deg, #818cf8, #6366f1)' : 'linear-gradient(90deg, #f59e0b, #f97316)'
                      }}
                    />
                  </div>
                  <div className="flex items-center justify-between mt-1.5">
                    <span className="text-[10px] text-gray-500">${fmtMoney(goal.current || 0)} / ${fmtMoney(goal.target)}</span>
                    ${goal.category && html`<span className="text-[10px] text-gray-500 bg-dark-600 px-1.5 py-0.5 rounded">${goal.category}</span>`}
                  </div>
                </div>`;
            })}
          </div>
      ...
      ${upcomingTasks.length > 0 && html`<div className="glass rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-bold text-white flex items-center gap-2">
              \u{1F4CB} Upcoming Tasks
            </h2>
          ...
          <div className="space-y-2">
            ${upcomingTasks.slice(0, 5).map(function(task) {
              ...
              return html`<div key=${task.id} className="${'flex items-center gap-3 p-3 rounded-xl transition ' + (isOverdue ? 'bg-rose-500/5 border border-rose-500/10' : 'bg-dark-700/50 hover:bg-dark-700')}">
                  ...
                  ${task.priority && html`<span className="${'text-[10px] font-medium px-2 py-0.5 rounded-full border shrink-0 ' + (priorityColors[task.priority] || priorityColors.low)}">
                      ${task.priority}
                    </span>`}
                </div>`;
            })}
      ...
      ${goals.length > 0 && html`<div className="glass rounded-2xl p-5">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-sm font-bold text-white flex items-center gap-2">
              \u{1F3C6} Goal Progress Overview
            </h2>
          </div>
          <div className="flex flex-wrap justify-center gap-6">
            ${goals.slice(0, 6).map(function(goal) {
              ...
              return html`<div key=${goal.id} className="flex flex-col items-center gap-2 w-28">
                  <div className="relative inline-flex items-center justify-center">
      ...
      ${habits.length === 0 && tasks.length === 0 && goals.length === 0 && html`<div className="glass rounded-2xl p-8 text-center">
